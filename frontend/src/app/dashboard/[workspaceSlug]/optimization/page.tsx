...
export default function PrescriptiveOptimizationHubPage() {
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-white tracking-tight">
            Prescriptive Optimization & Goal-Seeking Studio
          </h1>
          <p className="text-slate-400 mt-2 max-w-2xl text-sm leading-relaxed">
            Solve for optimal resource allocations and driver adjustments to achieve your Target KPI goals.
            Enforce min/max percentage bounds and evaluate marginal ROI efficiency ratios.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-indigo-950/50 border border-indigo-500/30 rounded-lg px-4 py-2.5">
            ...
            <span className="text-xs font-medium text-indigo-200">
              Goal-Seeking Inverse Elasticity • Simplex Resource Allocation
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-5 relative overflow-hidden">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className="font-semibold text-white text-base">Goal-Seeking Solver</h3>
          </div>
          <p className="text-slate-400 text-xs leading-relaxed">
            Specify a target dollar or percentage uplift goal and calculate the required driver
            allocations needed across high-efficiency segments.
          </p>
        </div>
        ...
        <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-5 relative overflow-hidden">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className="font-semibold text-white text-base">Resource Allocation</h3>
          </div>
          <p className="text-slate-400 text-xs leading-relaxed">
            Maximize enterprise KPI returns under a fixed budget cap by allocating resources
            proportional to segment-level ROI efficiency multiples.
          </p>
        </div>
        ...
        <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-5 relative overflow-hidden">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className="font-semibold text-white text-base">Constraint Bounds</h3>
          </div>
          <p className="text-slate-400 text-xs leading-relaxed">
            Enforce strict min/max percentage bounds (±25% or ±50%) on individual segments to prevent
            unrealistic edge-case allocations.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            Select a Workspace Dataset for Prescriptive Optimization
          </h2>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 bg-slate-900/40 border border-slate-800 rounded-xl">
            ...
            <p className="text-xs text-slate-400 mt-1">
              Upload a dataset in the Data Ingestion Studio to start building prescriptive action plans.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {datasets.map((ds) => (
              <div
                key={ds.id}
                onClick={() =>
                  router.push(`/dashboard/${workspaceSlug}/optimization/${ds.id}`)
                }
